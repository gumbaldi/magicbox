...
management action) — `settings` and `setup` are recognised arguments, routing straight to the
skill's settings menu and setup wizard. Use the code-for-queue skill and follow its steps from the
beginning; the block above is current context, not a replacement for the skill's own steps.
