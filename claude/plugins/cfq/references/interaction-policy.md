...
immediately — before `pfq`'s Step 1 / before `ifq`'s Step 1-3a preflight call — naming what this
...
batch/phase files in Step 10, `ifq` writes code in Step 4c) — resolve this before the first write
...
Before treating an open point as "routine" (`pfq` Step 4) or answering the closing "anything else
open?" check (`pfq` Step 6) as satisfied, explicitly enumerate every decision made autonomously
