...
Only read when the user picked "Thorough grilling" or "Grilling with docs" in **Start Block**. Use
**Start Block**'s preflight result directly — `planningPolicy.grillMode`/`.useMattpocockGrilling` —
no new `bin/cfq settings get` call:
...
**Closing Question** before writing any plans.

## With-Docs Path

Only on the third option from **Start Block**. The interview above runs unchanged; this adds the
written paper trail. Reuses **Start Block**'s `planningPolicy.useMattpocockGrilling` — no new call.
...
Adapted from Matt Pocock's skills (MIT) — <https://github.com/mattpocock/skills>.
