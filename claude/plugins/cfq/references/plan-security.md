...
exists yet → write the `plan/` entry via `note plan … - <<'EOF' … EOF` (body on stdin, never a
temp file: counts per severity, the fixable advisories, a note that a decision is still open); one
already exists → write nothing, say so in the status line instead. Never a question, never a phase
in this batch.
